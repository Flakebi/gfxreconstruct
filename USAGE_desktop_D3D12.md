...
Capture Single DispatchRays via Hotkey | GFXRECON_CAPTURE_DISPATCH_RAYS_ONLY | BOOL | When `true` and `GFXRECON_CAPTURE_TRIGGER` is set to a hotkey, holding the hotkey captures one `DispatchRays` per frame, each into its own `*_draw_call_*.gfxr` file. The layer re-arms automatically after each capture for as long as the key is held — release the key to stop. Useful when you want to capture specific raytracing dispatches but don't know their `(submit, commandlist, draw_call)` indices in advance. Default is: `false`.
